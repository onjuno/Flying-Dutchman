...
function createItemHTML(element, categoryType) {
    ...
    var additionalInfo = '';
        ...
        case 'cocktail':
            ...
            break;
        case 'special_alcohol':
            typeLabel = ' (Special Alcohol)';
            additionalInfo = '<p>Producer:' + element.producer +'</p>'+
                            '<p>Country:' + element.countryoforigin +'</p>' +
                            '<p>Type:' + element.category +'</p>'+                        
                            '<p>Strength:' + element.alcoholstrength +'%</p>' ;
            break;
        }
...
//Special Drinks tab function
document.getElementById('SpecialDrinks').addEventListener('click', function() {
// Prompt for password and check
const password = prompt("Enter the password to access Special Drinks:");
const correctPassword = "special123"; // Predefined password

if (password === correctPassword) {
    // Password is correct, show the "Special Drinks" section
    showSectionAndHighlightButton('specialDrinks', 'SpecialDrinks');
} else {
...
document.addEventListener("DOMContentLoaded", function() {
    // Locate the "Pay" button
    var payButton = document.getElementById("payButton");

    // Add event listener to the "Pay" button
    payButton.addEventListener("click", function() {
        // Prompt the user with a confirmation dialog
        var payMethod = confirm("Do you want to pay by your balance? If not, you'll pay at the bar.");

        // Reset the shopping cart and total amount regardless of payment method
        resetShoppingCart();
    ...
    // Function to reset the shopping cart and total amount
    function resetShoppingCart() {
        // Reset the content of the shopping cart
    ...
    // Function to update the balance display (optional)
    function updateBalanceDisplay() {
        var customerBalance = document.getElementById("customer-balance");
        // Update the balance display
        customerBalance.textContent = "Your Balance: $0.00"; // Update with the actual balance
    }
});
